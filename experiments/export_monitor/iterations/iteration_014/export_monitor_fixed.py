...
class HTPConsoleWriter(StepAwareWriter):
    ...
    def write_model_prep(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        self._print_header("📋 STEP 1/8: MODEL PREPARATION")
            ...
            f"({data.total_modules} modules, {data.total_parameters/1e6:.1f}M parameters)"
                ...
                self.console.print("⚙️ Strategy: HTP (Hierarchy-Preserving)")
    ...
    def write_input_gen(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if "input_generation" in data.steps:
            ...
            if "inputs" in step_data:
                                self.console.print(f"✅ Created onnx export config for {step_data.get('model_type')} with task {step_data.get('task')}")
                ...
                for name, spec in step_data['inputs'].items():
                    ...
                    dtype_str = spec.get('dtype', 'unknown')
                                        self.console.print(f"   • {name}: {shape_str} ({dtype_str})")
        return 1
    ...
    def write_hierarchy(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if data.execution_steps > 0:
                        self.console.print(f"🔄 Execution steps: {data.execution_steps}")
    ...
    def write_onnx_export(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        self._print_header("📦 STEP 4/8: ONNX EXPORT")
                self.console.print(f"🎯 Target file: {data.output_path}")
        
        if "onnx_export" in data.steps:
            step_data = data.steps["onnx_export"]
                        self.console.print("⚙️ Export config:")
                        self.console.print(f"   • opset_version: {step_data.get('opset_version', 17)}")
                        self.console.print(f"   • do_constant_folding: {step_data.get('do_constant_folding', True)}")
                        self.console.print("   • verbose: False")
            
            # Add input names if available
            if data.input_names:
                                self.console.print(f"   • input_names: {data.input_names}")
        
                self.console.print("✅ ONNX export completed successfully")
        return 1
    ...
    def write_tagger_creation(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if "tagger_creation" in data.steps:
            ...
            enable_fallback = step_data.get("enable_operation_fallback", False)
                        self.console.print(f"🔧 Operation fallback: {'enabled' if enable_fallback else 'disabled'}")
    ...
    def write_node_tagging(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if empty == 0:
                        self.console.print("✅ Empty tags: 0")
        else:
                        self.console.print(f"❌ Empty tags: {empty}")
    ...
    def write_tag_injection(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if data.embed_hierarchy_attributes:
                        self.console.print("🏷️ Hierarchy tag attributes: enabled")
                        self.console.print("✅ Tags injected into ONNX model successfully")
                        self.console.print(f"📄 Updated ONNX file: {data.output_path}")
        else:
                        self.console.print("⚠️ Tag injection skipped (clean ONNX mode)")
    ...
    def write_metadata_gen(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        self._print_header("📄 STEP 8/8: METADATA GENERATION")
                self.console.print("✅ Metadata file created successfully")
        
        if data.metadata_path:
                        self.console.print(f"📄 Metadata file: {data.metadata_path}")
    ...
    def write_complete(self, export_step: HTPExportStep, data: HTPExportData) -> int:
        ...
        if data.output_path:
                        self.console.print(f"   • ONNX model: {data.output_path}")
        if data.metadata_path:
                        self.console.print(f"   • Metadata: {data.metadata_path}")
        if data.report_path:
                        self.console.print(f"   • Report: {data.report_path}")
        else:
                        self.console.print("   • Report: disabled")
    ...
    def _print_hierarchy_tree(self, hierarchy: dict, max_lines: int | None = None) -> None:
        ...
        for i, line in enumerate(lines):
            if i >= max_lines:
                ...
                if remaining > 0:
                                        self.console.print(f"... and {remaining} more lines (truncated for console)")
                break
                        self.console.print(line)
        
        # Show line count summary
        if len(lines) <= max_lines:
                        self.console.print(f"(showing {len(lines)}/{len(lines)} lines)")
        else:
                        self.console.print(f"(showing {max_lines}/{len(lines)} lines)")
    ...
    def _print_top_nodes_by_hierarchy(self, tagged_nodes: dict[str, str]) -> None:
        ...
        if top_tags:
            ...
            for i, (tag, count) in enumerate(top_tags[:self.TOP_NODES_COUNT], 1):
                                self.console.print(f"{i:2d}. {tag}: {count} nodes")
    ...
    def _print_hierarchy_with_nodes(self, hierarchy: dict, tagged_nodes: dict[str, str], total_nodes: int) -> None:
        ...
        root_name = root_info.get("class_name", "Model")
                ...
                self.console.print(f"{root_name} ({total_nodes} ONNX nodes)")
        ...
        def print_module_with_nodes(path: str, level: int = 1, line_count: list | None = None):
            ...
            for child_path in children:
                ...
                indent = "│   " * (level - 1) + "├── "
                                self.console.print(f"{indent}{class_name}: {display_name} ({node_count} nodes)")
                line_count[0] += 1
                ...
                if tag in nodes_by_tag and level <= 3 and node_count > 0:
                    ...
                    for op_type, op_nodes in sorted_ops[:5]:  # Show top 5 operation types
                        ...
                        count = len(op_nodes)
                        if count > 1:
                                                        self.console.print(f"{op_indent}{op_type} ({count} ops)")
                        else:
                            # For single ops, show the full name
                                                        self.console.print(f"{op_indent}{op_type}: {op_nodes[0]}")
                        line_count[0] += 1
        ...
        if lines_shown < total_nodes:
                        self.console.print(f"... and {total_nodes - lines_shown} more lines (truncated for console)")
                self.console.print(f"(showing {lines_shown}/{total_nodes} lines)")
        ...
        for writer in self.writers:
            try:
                writer.write(step, self.data)
                                ...
                                self.console.print(f"Error in {writer.__class__.__name__}: {e}")
