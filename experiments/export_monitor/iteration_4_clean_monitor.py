#!/usr/bin/env python3
"""
Iteration 4: Clean, simplified export monitor that matches baseline exactly.
...
def create_clean_export_monitor():
    """Create a clean, simple export monitor."""
    
    code = '''"""
HTP Export Monitor - Clean, simple implementation for iteration 4.
No over-engineering, just the essentials to match baseline output.
"""
...
class HTPExportMonitor:
    """Simple, clean export monitor that outputs to console, metadata, and report."""
    ...
    def _print(self, msg: str = "", to_console: bool = True, to_buffer: bool = True) -> None:
        """Print to console and/or buffer."""
    ...
    def model_preparation(self, model_class: str, total_modules: int, total_parameters: int,
        ...
        self._print(Config.MESSAGES["export_target"].format(path=self.state.output_path))
        self._print(Config.MESSAGES["strategy"])
        
        if embed_hierarchy_attributes:
            self._print(Config.MESSAGES["hierarchy_enabled"])
        
        self._print(Config.MESSAGES["eval_mode"])
    ...
    def input_generation(self, model_type: str, task: str, inputs: dict) -> None:
        ...
        self._print(Config.MESSAGES["auto_inputs"].format(model=self.model_name))
        self._print(f"   • Model type: {model_type}")
        self._print(f"   • Auto-detected task: {task}")
        self._print(Config.MESSAGES["export_config"].format(model_type=model_type, task=task))
        self._print(Config.MESSAGES["generated_tensors"].format(count=len(inputs)))
        
        for name, spec in inputs.items():
            self._print(f"   • {name}: {spec['shape']} ({spec['dtype']})")
        ...
        if self.report_buffer:
            ...
            self.report_buffer.write(f"Method: auto_generated\\n")
    ...
    def hierarchy_building(self, hierarchy: dict, execution_steps: int) -> None:
        ...
        self._print(Config.MESSAGES["hierarchy_complete"])
        self._print(Config.MESSAGES["traced_modules"].format(count=len(hierarchy)))
        self._print(Config.MESSAGES["execution_steps"].format(count=execution_steps))
    ...
    def onnx_export(self, opset_version: int = 17, do_constant_folding: bool = True,
        ...
        self._print(Config.MESSAGES["target_file"].format(path=self.state.output_path))
        ...
        self._print(Config.MESSAGES["export_complete"])
    ...
    def tagger_creation(self, enable_operation_fallback: bool = False) -> None:
        ...
        self._print(Config.MESSAGES["tagger_created"])
        root_class = self.state.hierarchy.get("", {}).get("class_name", "Model")
        self._print(Config.MESSAGES["model_root_tag"].format(class_name=root_class))
    ...
    def node_tagging(self, total_nodes: int, tagged_nodes: dict, statistics: dict) -> None:
        ...
        self._print(Config.MESSAGES["tagging_complete"])
        self._print(Config.MESSAGES["coverage"].format(percent=self.state.coverage))
        self._print(Config.MESSAGES["tagged_nodes"].format(tagged=len(tagged_nodes), total=total_nodes))
        ...
        if empty == 0:
            self._print(Config.MESSAGES["empty_tags_ok"].format(count=empty))
        else:
            self._print(Config.MESSAGES["empty_tags_error"].format(count=empty))
    ...
    def tag_injection(self) -> None:
        ...
        if self.state.embed_hierarchy_attributes:
            self._print(Config.MESSAGES["tag_injection_enabled"])
            self._print(Config.MESSAGES["tag_injection_complete"])
            self._print(Config.MESSAGES["updated_file"].format(path=self.state.output_path))
    ...
    def metadata_generation(self) -> None:
        ...
        self._print(Config.MESSAGES["metadata_created"])
        self._print(Config.MESSAGES["metadata_file"].format(path=self.state.metadata_path))
        
        # Write metadata
        self._write_metadata()
    
    # Final summary
    def complete(self, export_time: float = None) -> None:
        ...
        self._print(Config.MESSAGES["export_success"].format(time=export_time))
        self._print(Config.MESSAGES["export_stats"])
        ...
        if empty == 0:
            self._print("   • Empty tags: 0 ✅")
        else:
            self._print(f"   • Empty tags: {empty} ❌")
        
        self._print(f"\\n{Config.MESSAGES['output_files']}")
    ...
    def _print_hierarchy_tree(self, hierarchy: dict) -> None:
        """Print module hierarchy as tree."""
        self._print("\\n🌳 Module Hierarchy:")
        self._print(Config.MINOR_SEP)
        ...
        # Print tree
        def print_tree(path: str, prefix: str = "", line_count: list = None):
            ...
            for i, child_path in enumerate(children):
                ...
                self._print(f"{branch}{class_name}: {child_path}")
                line_count[0] += 1
    ...
    def _print_hierarchy_with_nodes(self) -> None:
        ...
        root_name = root_info.get("class_name", "Model")
        self._print(f"{root_name} ({self.state.total_nodes} ONNX nodes)")
        ...
        def print_module_nodes(path: str, level: int = 1, prefix: str = "", line_count: list = None):
            if line_count is None:
                line_count = [1]
            ...
            for i, child_path in enumerate(children):
                ...
                branch = prefix + ("└── " if is_last else "├── ")
                self._print(f"{branch}{class_name}: {child_path} ({node_count} nodes)")
                line_count[0] += 1
                ...
                if tag in nodes_by_tag and level <= 3 and node_count > 0:
                    ...
                    for j, (op_type, op_nodes) in enumerate(sorted_ops[:5]):
                        ...
                        if len(op_nodes) > 1:
                            self._print(f"{op_branch}{op_type} ({len(op_nodes)} ops)")
                        else:
                            self._print(f"{op_branch}{op_type}: {op_nodes[0]}")
                        line_count[0] += 1
        ...
        if line_counter[0] >= Config.NODE_TREE_MAX:
            ...
            self._print(f"... and {remaining} more lines (truncated for console)")
        self._print(f"(showing {min(line_counter[0]-1, Config.NODE_TREE_MAX)}/{self.state.total_nodes-1} lines)")
...
'''
    ...
    # Save the file
    output_path = Path("/home/zhengte/modelexport_allmodels/experiments/export_monitor/export_monitor_clean.py")
    with open(output_path, 'w') as f:
        f.write(code)
    
    print(f"✅ Created clean export monitor at {output_path}")
...
def main():
    """Create iteration 4 clean monitor."""
    print("🔧 ITERATION 4 - Clean Export Monitor")
    print("=" * 60)
    
    print("\n📝 Goals:")
    print("1. Simple, clean code - no over-engineering")
    print("2. All magic numbers/strings in Config class")
    print("3. Match baseline output exactly")
    print("4. No complicated inheritance or abstractions")
    print("5. Just the essentials")
    
    # Create the clean monitor
    monitor_path = create_clean_export_monitor()
    
    print("\n✅ Clean export monitor created!")
    print("\nNext: Test with actual data to ensure it matches baseline")
