...
class TestRunner:
    ...
    def ci_tests(self) -> bool:
        ...
        cmd = [
            "python", "-m", "pytest", 
            "-m", "not (slow or requires_gpu or requires_models)",
            "-v", "--tb=short",
...
Test Categories:
  ...
  ⚡ Performance Tests: Speed/memory benchmarks (20 min)
    ...
    test_group.add_argument("--smoke", action="store_true", 
                           help="Run smoke tests (5 min)")
    test_group.add_argument("--sanity", action="store_true", 
                           help="Run sanity tests (15 min)")
    test_group.add_argument("--unit", action="store_true", 
                           help="Run unit tests (30 min)")
    test_group.add_argument("--integration", action="store_true", 
                           help="Run integration tests (45 min)")
    test_group.add_argument("--performance", action="store_true", 
                           help="Run performance tests (20 min)")
    test_group.add_argument("--multimodal", action="store_true", 
                           help="Run multimodal tests only")
    test_group.add_argument("--fast", action="store_true", 
                           help="Run all except slow tests")
    test_group.add_argument("--ci", action="store_true", 
                           help="Run CI-friendly test subset")
    test_group.add_argument("--all", action="store_true", 
                           help="Run all tests")
    test_group.add_argument("--discover", action="store_true", 
                           help="Show test discovery without running")
    test_group.add_argument("--progressive", action="store_true", 
                           help="Run progressive validation workflow")
