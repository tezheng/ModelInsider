...
from modelexport.inference.types import TensorType
...
class MockONNXModel:
    ...
    def add_input(
        self, name: str, shape: list[int], dtype: int | TensorType = TensorType.FLOAT32
    ) -> MockONNXModel:
...
def create_text_onnx_model(
    ...
    vocab_size: int = BERT_VOCAB_SIZE,
) -> onnx.ModelProto:
    """Create a mock BERT-like text model."""
    builder = MockONNXModel(model_name)

    return (
        builder.add_input("input_ids", [batch_size, sequence_length], TensorType.INT64)
        ...
        .add_output(
            "last_hidden_state", [batch_size, sequence_length, 768], TensorType.FLOAT32
        )
...
def create_image_onnx_model(
    ...
    return (
        builder.add_input(
            "pixel_values", [batch_size, channels, height, width], TensorType.FLOAT32
        )
...
def create_audio_onnx_model(
    ...
    return (
        builder.add_input(input_name, input_shape, TensorType.FLOAT32)
        .add_input("attention_mask", [batch_size, sequence_length], TensorType.INT64)
        .add_output(
            "last_hidden_state",
            [batch_size, sequence_length // 320, 768],
            TensorType.FLOAT32,
        )
...
def create_multimodal_onnx_model(
    ...
    return (
        builder.add_input("input_ids", [batch_size, text_seq_length], TensorType.INT64)
...
def create_mock_base_processor(processor_type: str = "tokenizer") -> Mock:
    ...
    if processor_type == "tokenizer":
        ...
        def mock_call(*args, **kwargs):
            ...
            data = {
                "input_ids": np.array(
                    [[101, 7592, 2088, 102] + [0] * (max_length - 4)]
                ),
        ...
        try:
            ...
            mock_processor.__class__ = type(
                "MockImageProcessor", (BaseImageProcessor,), {}
            )
        except ImportError:
            # Fallback if transformers not available
        ...
        try:
            ...
            mock_processor.__class__ = type(
                "MockFeatureExtractor", (FeatureExtractionMixin,), {}
            )
        except ImportError:
            # Fallback if transformers not available
        ...
        try:
            ...
            mock_processor.__class__ = type(
                "MockVideoProcessor", (BaseImageProcessor,), {}
            )
        except ImportError:
            # Fallback if transformers not available
            mock_processor.__class__.__name__ = "BaseImageProcessor"
        ...
        try:
            ...
            mock_processor.__class__ = type(
                "MockMultimodalProcessor", (ProcessorMixin,), {}
            )
        except ImportError:
            # Fallback if transformers not available
...
def save_onnx_model_to_temp(
    model: onnx.ModelProto, filename: str | None = None
) -> Path:
    ...
    modality_configs = {
        ...
        "clip": {
            "text": {
                ...
                "batch_size": 1,
                "sequence_length": 77,
                "tensors": [{"name": "input_ids", "shape": [1, 77], "dtype": 7}],
            },
            "image": {
                "type": "image",
                "batch_size": 1,
...
def create_performance_test_data(
    modality: str = "text", num_samples: int = 100
