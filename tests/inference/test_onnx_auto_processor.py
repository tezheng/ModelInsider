...
from modelexport.inference.processors import (
    BaseONNXProcessor,
    ONNXAudioProcessor,
    ONNXImageProcessor,
    ONNXProcessor,
...
from modelexport.inference.types import (
    ModalityConfig,
    ModalityType,
    ONNXModelLoadError,
    ONNXProcessorError,
...
class TestONNXAutoProcessor:
    ...
    def test_multimodal_detection_clip(
        self, clip_onnx_model, mock_multimodal_processor
    ):
        ...
        with patch(
            ...
            try:
                processor = ONNXAutoProcessor.from_model(
                    model_path, base_processor=mock_multimodal_processor
                )
                assert processor is not None
    ...
    def test_all_five_processor_types(self):
        ...
        test_models = {
            "text": (
                create_text_onnx_model("bert_sanity"),
                create_mock_base_processor("tokenizer"),
            ),
            "image": (
                create_image_onnx_model("vit_sanity"),
                create_mock_base_processor("image_processor"),
            ),
            "audio": (
                create_audio_onnx_model("wav2vec2_sanity"),
                create_mock_base_processor("feature_extractor"),
            ),
            "video": (
                create_video_onnx_model("videomae_sanity"),
                create_mock_base_processor("video_processor"),
            ),
            "multimodal": (
                create_multimodal_onnx_model("clip_sanity"),
                create_mock_base_processor("multimodal"),
            ),
        }
        ...
        for modality, (model, base_processor) in test_models.items():
            ...
            with patch(
                ...
                processor = ONNXAutoProcessor.from_model(
                    model_path, base_processor=base_processor
                )
    ...
    def test_fixed_shape_enforcement(self, bert_onnx_model, mock_tokenizer):
        ...
        with patch(
            ...
            output2 = processor(
                "This is a much longer sentence that should be padded or truncated"
            )

            # Both should have same shape
            for key in output1:
                if key in output2:
                    assert output1[key].shape == output2[key].shape, (
    ...
    def test_metadata_priority_order(self):
        ...
        processor_metadata = ONNXAutoProcessor._extract_processor_metadata(
            model, model_path
        )
    ...
    def test_backward_compatibility_interface(self, bert_onnx_model, mock_tokenizer):
        ...
        with patch(
            ...
            auto_processor = ONNXAutoProcessor.from_model(
                model_path, base_processor=mock_tokenizer
            )
    ...
    def test_modality_detection_by_name(self):
        ...
        for tensor_name, expected_modality in test_cases:
            ...
            assert detected == expected_modality, (
                f"Failed for {tensor_name}: expected {expected_modality}, got {detected}"
            )
    ...
    def test_modality_detection_by_shape(self):
        ...
        for shape, expected_modality in test_cases:
            ...
            assert detected == expected_modality, (
                f"Failed for {shape}: expected {expected_modality}, got {detected}"
            )
    ...
    def test_shape_compatibility_checking(self):
        ...
        spec = TensorSpec(
            "test", [1, -1, 128], TensorType.FLOAT32
        )  # Dynamic middle dimension
    ...
    def test_end_to_end_bert_processor(self, mock_tokenizer):
        ...
        bert_model = create_text_onnx_model(
            "bert_e2e", batch_size=1, sequence_length=512
        )
        ...
        with patch(
            ...
            # Test from_model
            processor = ONNXAutoProcessor.from_model(
                bert_path, base_processor=mock_tokenizer
            )
            ...
            expected_shapes = {"input_ids": [1, 512], "attention_mask": [1, 512]}
            # Only check shapes for tensors that exist
            actual_shapes = {
                k: list(v.shape) for k, v in output.items() if k in expected_shapes
            }
    ...
    def test_end_to_end_clip_processor(self, mock_multimodal_processor):
        ...
        with patch(
            ...
            processor = ONNXAutoProcessor.from_model(
                clip_path, base_processor=mock_multimodal_processor
            )
    ...
    def test_processor_from_model_directory_workflow(self):
        ...
        with patch(
            ...
            mock_auto.from_pretrained.return_value = create_mock_base_processor(
                "tokenizer"
            )
    ...
    def test_processor_creation_speed(self, performance_text_data):
        ...
        with patch(
            ...
            assert stats["avg_time"] < 0.1, (
                f"Processor creation too slow: {stats['avg_time']:.3f}s (target: <0.1s)"
            )
    ...
    def test_text_preprocessing_speed(self, performance_text_data):
        ...
        bert_model = create_text_onnx_model(
            "perf_text", batch_size=1, sequence_length=128
        )
        model_path = save_onnx_model_to_temp(bert_model)
        mock_tokenizer = create_mock_base_processor("tokenizer")
        ...
        with patch(
            ...
            assert stats["avg_time"] < 0.01, (
                f"Text preprocessing too slow: {stats['avg_time']:.4f}s per text"
            )
    ...
    def test_image_preprocessing_speed(self, performance_image_data):
        ...
        with patch(
            ...
            processor = ONNXAutoProcessor.from_model(
                model_path, base_processor=mock_processor
            )
    ...
    def test_error_message_quality(self):
        ...
        try:
            ...
            assert "Batch size must be positive" in str(
                e
            ) or "At least one tensor" in str(e)
    ...
    def test_concurrent_processing(self):
        ...
        with patch(
            ...
            assert errors_queue.empty(), (
                f"Concurrent processing errors: {list(errors_queue.queue)}"
            )
    ...
    def test_with_all_fixtures(
        ...
        for model, processor, modality_name in models_and_processors:
            ...
            with patch(
                ...
                onnx_processor = ONNXAutoProcessor.from_model(
                    model_path, base_processor=processor
                )
                assert onnx_processor is not None
                print(
                    f"✓ Successfully created processor for {modality_name} using fixtures"
                )
