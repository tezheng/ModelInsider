...
def sample_htp_metadata(tmp_path):
    ...
    metadata = {
        "export_data": {
            ...
            "tagged_nodes": {
                "Add_24": {"hierarchy_tag": "embeddings", "op_type": "Add"},
                "MatMul_42": {
...
def alternative_metadata_format(tmp_path):
    ...
    metadata = {
        "hierarchy": {
            "": {"class_name": "GPT2Model", "execution_order": 0},
            "transformer": {"class_name": "GPT2MainLayer", "execution_order": 1},
        },
        "node_tags": {"Add_10": {"tag": "transformer", "op_type": "Add"}},
        "modules": {
            "": {"class_name": "GPT2Model", "module_type": "GPT2Model"},
            "transformer": {
...
class TestMetadataReader:
    ...
    def test_get_all_modules(self, sample_htp_metadata):
        ...
        assert modules.index("encoder/layer/0/attention") > modules.index(
            "encoder/layer/0"
        )
    ...
    def test_string_format_node_tags(self, tmp_path):
        ...
        assert (
            reader.get_node_hierarchy_tag("/embeddings/Constant")
            == "/BertModel/BertEmbeddings"
        )
        assert (
            reader.get_node_hierarchy_tag("/embeddings/Add")
            == "/BertModel/BertEmbeddings"
        )
        assert (
            reader.get_node_hierarchy_tag("/encoder/layer/0/attention/MatMul")
            == "/BertModel/BertEncoder/ModuleList/BertLayer/BertAttention"
        )
        assert reader.get_node_hierarchy_tag("NonExistent") is None
    ...
    def test_partial_metadata_handling(self, tmp_path):
        ...
        metadata = {
            "hierarchy_data": {"module1": {"class_name": "TestModule"}}
