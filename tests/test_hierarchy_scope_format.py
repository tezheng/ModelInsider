...
class TestHierarchyScopeFormat:
    ...
    def test_real_model_hierarchy_scope(self):
        ...
        builder.trace_model_execution(
            model, {"input_ids": model.dummy_inputs["input_ids"]}
        )
    ...
    def test_rich_tree_shows_full_scope(self):
        ...
        hierarchy = {
            ...
            "encoder.layer.0": {
                "class_name": "Layer",
                "traced_tag": "/Model/Encoder/Layer.0",
            },
        }
        ...
        assert (
            ": 0" not in tree_str or ": 0 " not in tree_str
        )  # Avoid false positive with counts
