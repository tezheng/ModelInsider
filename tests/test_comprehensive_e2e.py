...
class TestModelArchitecture:
    ...
    def test_model_export(self, model_id):
        ...
        try:
            ...
            if (
                "bounding boxes" in error_msg
                and model_config.get("special_handling") == "coordinate_inputs"
            ):
                pytest.fail(f"SAM model {model_id} failed unexpectedly: {error_msg}")
            elif "DynamicCache" in error_msg:
                pytest.fail(
                    f"Model {model_id} has cache issue despite use_cache=False: {e}"
                )
            else:
    ...
    def test_llm_cache_handling(self):
        ...
        for model_id, model_config in LLM_MODELS.items():
            if model_config.get("use_cache") is False:
                ...
                assert model_config["use_cache"] is False, (
                    f"{model_id} should have use_cache=False"
                )
...
class TestCrossArchitecture:
    ...
    def test_model_diversity_coverage(self):
        ...
        assert len(architectures) >= 4, (
            f"Should test multiple architectures, got {architectures}"
        )
        ...
        assert classic_count >= 3, (
            f"Should have at least 3 classic models, got {classic_count}"
        )
    ...
    def test_export_time_scaling(self):
        ...
        for model_id, stats in timing_results.items():
            assert stats["time"] < 120.0, (
                f"{model_id} export took too long: {stats['time']:.2f}s"
            )
