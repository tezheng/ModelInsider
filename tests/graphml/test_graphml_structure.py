...
class TestGraphMLStructure:
    ...
    def test_key_definitions_match_baseline(self, tmp_path):
        ...
        assert key_map["meta0"] == ("graph", "source_onnx_file"), (
            "Wrong definition for meta0"
        )

        assert "meta2" in key_map, "Missing graph format_version key (meta2)"
        assert key_map["meta2"] == ("graph", "format_version"), (
            "Wrong definition for meta2"
        )

        assert "meta3" in key_map, "Missing graph export_timestamp key (meta3)"
        assert key_map["meta3"] == ("graph", "export_timestamp"), (
            "Wrong definition for meta3"
        )
    ...
    def test_node_name_attribute(self, tmp_path):
        ...
        for node in nodes[:3]:  # Check first few nodes
            ...
            assert op_type_data is not None, (
                f"Node {node_id} missing op_type data (key n0)"
            )
    ...
    def test_graph_metadata_attributes(self, tmp_path):
        ...
        assert format_version.text == GRAPHML_VERSION, (
            f"Wrong format version: {format_version.text}"
        )
...
class TestMissingFeatures:
    ...
    def test_node_attributes_json_storage(self, tmp_path):
        ...
        op_node = None
        for node in nodes:
            if not node.get("id").startswith("input_") and not node.get(
                "id"
            ).startswith("output_"):
                op_node = node
                break
        ...
        assert attrs.get("module_type") == "Linear", (
            f"Expected module_type 'Linear', got {attrs.get('module_type')}"
        )
        assert attrs.get("execution_order") == 42, (
            f"Expected execution_order 42, got {attrs.get('execution_order')}"
        )
    ...
    def test_execution_order_extraction(self, tmp_path):
        ...
        nodes_with_order = [
            n for n in graph_data.nodes if n.execution_order is not None
        ]
        ...
        assert sorted(orders) == [0, 1, 2], (
            f"Expected orders [0, 1, 2], got {sorted(orders)}"
        )
...
class TestQAPerspective:
    ...
    def test_large_model_performance(self, tmp_path):
        ...
        layers = []
        for _i in range(20):
            layers.extend([nn.Linear(100, 100), nn.ReLU(), nn.BatchNorm1d(100)])
