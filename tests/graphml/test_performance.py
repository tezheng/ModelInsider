...
class TestONNXToGraphMLPerformance:
    ...
    def test_large_model_conversion_time(self, large_onnx_model):
        ...
        assert conversion_time < 60.0, (
            f"Conversion took {conversion_time:.2f}s, expected <60s"
        )
        ...
        print(
            f"Performance metrics - Time: {conversion_time:.2f}s, "
    ...
    def test_large_model_memory_usage(self, large_onnx_model):
        ...
        assert stats["nodes"] > 0, (
            f"Expected >0 nodes for large model, got {stats['nodes']}"
        )

        print(
            f"Memory metrics - Baseline: {baseline_memory:.1f}MB, "
            f"Peak: {peak_memory:.1f}MB, Increase: {memory_increase:.1f}MB"
        )

    def test_scalability_across_model_sizes(
        self, simple_onnx_model, medium_onnx_model, large_onnx_model
    ):
        ...
        assert large_time < simple_time * 100, (
            "Large model time should not be >100x simple model"
        )

        # Log scaling results
        for result in results:
            print(
                f"{result['name']} model - Time: {result['time']:.3f}s, "
    ...
    def test_hierarchical_converter_performance(self, large_onnx_model):
        ...
        try:
            ...
            assert base_time < 60.0, (
                f"Base converter took {base_time:.2f}s, expected <60s"
            )
            assert hierarchical_time < 60.0, (
                f"Hierarchical converter took {hierarchical_time:.2f}s, expected <60s"
            )

            # Hierarchical converter may be slower due to additional processing
            time_ratio = hierarchical_time / base_time
            assert time_ratio < 200.0, (
                f"Hierarchical converter {time_ratio:.1f}x slower than base - performance issue detected"
            )

            # Both outputs should be valid
            assert len(base_output) > 0  # base_output is string
            assert (
                isinstance(hierarchical_output, dict)
                and "graphml" in hierarchical_output
            )  # hierarchical_output is dict

            print(
                f"Converter comparison - Base: {base_time:.3f}s, "
    ...
    def test_complex_hierarchy_handling(self, medium_onnx_model):
        ...
        try:
            ...
            assert conversion_time < 30.0, (
                f"Hierarchical conversion took {conversion_time:.2f}s, expected <30s"
            )
            ...
            compound_nodes = [
                node for node in all_nodes if node.get("id", "").startswith("module_")
            ]

            print(
                f"Hierarchical performance - Time: {conversion_time:.3f}s, "
    ...
    @pytest.mark.slow
    def test_stress_conversion_multiple_models(
        self, simple_onnx_model, medium_onnx_model
    ):
        ...
        for i in range(10):
            ...
            assert conversion_time < 10.0, (
                f"Conversion {i + 1} took {conversion_time:.2f}s, expected <10s"
            )
        ...
        print(
            f"Stress test - Total: {total_time:.2f}s, Average: {avg_time:.3f}s per conversion"
        )

        # Total time should be reasonable
        assert total_time < 60.0, (
            f"10 conversions took {total_time:.2f}s, expected <60s total"
        )
