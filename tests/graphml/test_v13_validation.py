...
def simple_onnx_model():
    ...
    add_node = helper.make_node(
        "Add", inputs=["input1", "input2"], outputs=["output"], name="add_node"
    )
...
def valid_v13_graphml():
    ...
    # Add graph
    graph = ET.SubElement(
        graphml, "graph", attrib={"id": "TestModel", "edgedefault": "directed"}
    )
...
class TestSchemaValidation:
    ...
    def test_missing_required_keys(self, tmp_path):
        ...
        graphml = ET.Element(
            "graphml", attrib={"xmlns": "http://graphml.graphdrawing.org/xmlns"}
        )
...
class TestSemanticValidation:
    ...
    def test_edge_connectivity(self, valid_v13_graphml, tmp_path):
        ...
        ET.SubElement(
            graph, "edge", attrib={"source": "nonexistent_node", "target": "add_node"}
        )
...
class TestRoundTripValidation:
    ...
    def test_round_trip_accuracy(self, simple_onnx_model, tmp_path):
        ...
        htp_metadata = {
            "model": {"class_name": "TestModel", "name_or_path": "test"},
            "modules": {},
            "nodes": {},
        }
        ...
        converter = ONNXToGraphMLConverter(
            htp_metadata_path=str(htp_file), validate_output=False
        )
    ...
    def test_parameter_preservation(self, simple_onnx_model, tmp_path):
        ...
        weight = onnx.numpy_helper.from_array(
            np.array([[1.0, 2.0], [3.0, 4.0]], dtype=np.float32), name="weight"
        )
