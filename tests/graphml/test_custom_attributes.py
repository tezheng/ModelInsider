...
class TestCustomAttributeHandling:
    ...
    def test_custom_attributes_preserved_in_graphml(self, sample_htp_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            converter = ONNXToGraphMLConverter(htp_metadata_path=str(metadata_path))
            ...
            for node in root.findall(".//gml:node", namespaces):
                ...
                if hierarchy_elem is not None and hierarchy_elem.text:
                    nodes_with_hierarchy.append(
                        {"id": node.get("id"), "hierarchy_tag": hierarchy_elem.text}
                    )

            # Verify we found nodes with custom hierarchy attributes
            assert len(nodes_with_hierarchy) > 0, (
                "No nodes with hierarchy_tag found in GraphML"
            )
            ...
            module_tags = [
                tag for tag in hierarchy_tags if tag.startswith("/BertModel/")
            ]
            assert len(module_tags) > 0, (
                f"No module hierarchy tags found. Got: {hierarchy_tags[:5]}"
            )

            print(
                f"✅ Found {len(nodes_with_hierarchy)} nodes with hierarchy_tag custom attributes"
            )
    ...
    def test_custom_attributes_filtered_from_onnx_conversion(self, sample_htp_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Convert to GraphML
            result = onnx_to_graphml.convert(
                str(sample_onnx_path), str(temp_path / "test")
            )
            graphml_path = result["graphml"]
            ...
            graphml_to_onnx.convert(
                graphml_path, str(reconstructed_onnx_path), validate=False
            )
                ...
                for attr in node.attribute:
                    if attr.name in custom_attr_names:
                        nodes_with_custom_attrs.append(
                            {"node_name": node.name, "attr_name": attr.name}
                        )
            ...
            print(
                "✅ Custom attributes correctly filtered out during GraphML→ONNX conversion"
            )
    ...
    def test_valid_onnx_attributes_preserved(self, sample_htp_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"✅ {preserved_count} valid ONNX attributes preserved during round-trip"
            )
    ...
    def test_custom_attribute_filtering_logic(self):
        ...
        for attr in custom_attrs:
            ...
            assert is_custom, (
                f"Custom attribute '{attr}' should be recognized as custom"
            )
        ...
        for attr_name in valid_onnx_attrs:
            ...
            assert not is_custom, (
                f"Valid ONNX attribute '{attr_name}' should not be marked as custom"
            )
        ...
        for attr_name in custom_vendor_attrs:
            is_custom = converter._is_custom_attribute(attr_name)
            assert not is_custom, (
                f"Vendor attribute '{attr_name}' should be preserved (not marked as custom)"
            )
...
def sample_htp_data():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = runner.invoke(
            cli,
            ["export", "--model", "prajjwal1/bert-tiny", "--output", str(onnx_path)],
        )
