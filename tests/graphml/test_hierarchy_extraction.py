...
class HierarchicalTestModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.ModuleDict(
            {
                "embeddings": nn.Sequential(nn.Embedding(100, 64), nn.LayerNorm(64)),
...
class TestRealWorldScenarios:
    ...
    def test_bert_model_structure(self, tmp_path):
        ...
        class SimpleBERT(nn.Module):
            def __init__(self):
                ...
                self.encoder = nn.ModuleList(
                    [nn.TransformerEncoderLayer(128, 4, 512) for _ in range(2)]
                )
        ...
        assert (
            "LayerNorm" in graphml_str or "ReduceMean" in graphml_str
        )  # LayerNorm ops
