    ...
    def test_resnet_export_timestamps(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for step_name in problematic_steps:
                ...
                assert "timestamp" in steps[step_name], f"Step {step_name} is missing timestamp"
                
                # Verify timestamp format
                timestamp = steps[step_name]["timestamp"]
                assert timestamp.endswith("Z"), f"Step {step_name} timestamp not in ISO format: {timestamp}"
                assert "T" in timestamp, f"Step {step_name} timestamp missing T separator: {timestamp}"