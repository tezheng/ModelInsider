...
class SimpleHierarchicalModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.embeddings = nn.Sequential(nn.Linear(10, 20), nn.ReLU(), nn.Dropout(0.1))

        self.encoder = nn.Sequential(
            nn.Linear(20, 15), nn.ReLU(), nn.Linear(15, 10), nn.ReLU()
        )
...
def analyze_enhanced_model(onnx_path: str):
    ...
    print(
        f"Nodes with module info: {nodes_with_module_info}/{len(onnx_model.graph.node)}"
    )
...
def test_hierarchy_grouping(sample_onnx_path):
    ...
    for top_module in top_level_modules:
        ...
        child_modules = [
            name for name in hierarchy if name.startswith(top_module + ".")
        ]
        ...
        print(
            f"  {top_module}: {total_ops} operations (across {len(all_modules)} modules)"
        )
