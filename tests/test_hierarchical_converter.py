...
from modelexport.graphml.onnx_to_graphml_converter import (
    ONNXToGraphMLConverter as HierarchicalGraphMLConverter,
)
...
def sample_onnx_model(tmp_path):
    ...
    weight_init = helper.make_tensor(
        name="weight", data_type=TensorProto.FLOAT, dims=[3, 4], vals=[0.1] * 12
    )

    # Create nodes
    add_node = helper.make_node(
        "Add", inputs=["input", "bias"], outputs=["add_out"], name="Add_1"
    )

    matmul_node = helper.make_node(
        "MatMul", inputs=["add_out", "weight"], outputs=["output"], name="MatMul_2"
    )
...
def hierarchical_htp_metadata(tmp_path):
    ...
    metadata = {
        ...
        "tagged_nodes": {
            "Add_1": {"hierarchy_tag": "/TestModel/layer1", "op_type": "Add"},
            "MatMul_2": {"hierarchy_tag": "/TestModel/layer2", "op_type": "MatMul"},
        },
    }
...
def deep_hierarchy_metadata(tmp_path):
    ...
    metadata = {
        ...
        "tagged_nodes": {
            "Add_1": {"hierarchy_tag": "/DeepModel/encoder/layer/0", "op_type": "Add"},
            "MatMul_2": {
...
class TestHierarchicalGraphMLConverter:
    ...
    def test_init_with_valid_metadata(self, hierarchical_htp_metadata):
        ...
        converter = HierarchicalGraphMLConverter(
            htp_metadata_path=str(hierarchical_htp_metadata), exclude_initializers=True
        )
    ...
    def test_init_with_missing_metadata(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError):
            HierarchicalGraphMLConverter(htp_metadata_path=str(missing_path))

    def test_convert_basic_hierarchy(
        self, sample_onnx_model, hierarchical_htp_metadata
    ):
    ...
    def test_hierarchy_tag_assignment(
        self, sample_onnx_model, hierarchical_htp_metadata
    ):
    ...
    def test_compound_node_structure(
        self, sample_onnx_model, hierarchical_htp_metadata
    ):
        ...
        main_graph_nodes = main_graph.findall(
    ...
    def test_deep_hierarchy(self, sample_onnx_model, deep_hierarchy_metadata):
        ...
        converter = HierarchicalGraphMLConverter(
            htp_metadata_path=str(deep_hierarchy_metadata), exclude_initializers=True
        )
    ...
    def test_graphml_metadata_inclusion(
        self, sample_onnx_model, hierarchical_htp_metadata
    ):
        ...
        data_elements = main_graph.findall(
            "./{http://graphml.graphdrawing.org/xmlns}data"
        )
        ...
        non_empty_data = [
            d for d in data_elements if d.text and len(d.text.strip()) > 0
        ]
    ...
    def test_fallback_to_flat_structure(self, sample_onnx_model, tmp_path):
        ...
        converter = HierarchicalGraphMLConverter(
            htp_metadata_path=str(metadata_path), exclude_initializers=True
        )
    ...
    def test_module_class_name_in_compounds(
        self, sample_onnx_model, hierarchical_htp_metadata
    ):
        ...
        for compound_node in compound_nodes:
            data_elements = compound_node.findall(
                ".//{http://graphml.graphdrawing.org/xmlns}data"
            )
            op_types = [
                d.text for d in data_elements if d.get("key") == "d0"
            ]  # op_type key
