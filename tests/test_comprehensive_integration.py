...
class TestComprehensiveIntegration:
    ...
    def test_config_file_vs_manual_specs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                config_result["hierarchy_modules"] == auto_result["hierarchy_modules"]
            )
    ...
    def test_different_sequence_lengths(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for seq_len in test_lengths:
                ...
                assert (
                    result["hierarchy_modules"] == 18
                )  # BERT tiny has 18 HF modules (torch.nn excluded per MUST-002)
    ...
    def test_input_generator_standalone(self):
        ...
        vision_specs = {"pixel_values": {"shape": [1, 3, 224, 224], "dtype": "float"}}
    ...
    def test_export_config_format(self):
        ...
        assert len(inputs) == 3
        assert all(
            name in inputs for name in ["input_ids", "token_type_ids", "attention_mask"]
        )
        assert all(tensor.shape == (1, 128) for tensor in inputs.values())
        assert all(tensor.dtype == torch.int64 for tensor in inputs.values())

        # Check ranges
        assert torch.all(inputs["input_ids"] >= 0) and torch.all(
            inputs["input_ids"] <= 1000
        )
        assert torch.all(inputs["token_type_ids"] >= 0) and torch.all(
            inputs["token_type_ids"] <= 1
        )
        assert torch.all(inputs["attention_mask"] >= 0) and torch.all(
