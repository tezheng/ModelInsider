...
class TestHTPExportMonitor:
    ...
    def test_traced_modules_in_export_summary(self):
        ...
        with patch("rich.console.Console.print") as mock_print:
            ...
            assert (
                "📊 Export Summary:" in full_output or "Export Summary:" in full_output
            )
            ...
            assert match is not None, (
                f"'Traced modules: X/Y' format not found in output:\n{full_output}"
            )
            ...
            assert traced <= total, (
                f"Traced modules ({traced}) should be <= total modules ({total})"
            )
