        ...
        self.data.hierarchy = HierarchyData(
            hierarchy={
                ...
                "embeddings": ModuleInfo(
                    class_name="BertEmbeddings", traced_tag="/BertModel/embeddings"
                ),
                "encoder": ModuleInfo(
                    class_name="BertEncoder", traced_tag="/BertModel/encoder"
                ),
                "encoder.layer.0": ModuleInfo(
                    class_name="BertLayer", traced_tag="/BertModel/encoder/layer.0"
                ),
            },
            execution_steps=42,
        )
    ...
    def test_hierarchy_truncation(self):
        ...
        for i in range(40):
            hierarchy[f"layer{i}"] = ModuleInfo(
                class_name=f"Layer{i}", traced_tag=f"/Model/layer{i}"
            )
