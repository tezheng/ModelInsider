...
class TestGraphMLIntegration:
    ...
    def test_hierarchical_conversion_with_bert_tiny(
        self, bert_tiny_model, bert_tiny_htp_metadata
    ):
        """Test hierarchical GraphML conversion with bert-tiny model and HTP metadata."""
        converter = ONNXToGraphMLConverter(
            htp_metadata_path=str(bert_tiny_htp_metadata), exclude_initializers=True
        )
        ...
        nested_graphs = root.findall(
            ".//{http://graphml.graphdrawing.org/xmlns}graph[@id!='G']"
        )
    ...
    def test_compound_node_hierarchy_validation(
        self, bert_tiny_model, bert_tiny_htp_metadata
    ):
    ...
    def test_node_tagging_validation(self, bert_tiny_model, bert_tiny_htp_metadata):
        ...
        op_nodes = [
            n
            for n in all_nodes
            if not n.get("id", "").startswith(("module_", "input_", "output_"))
        ]
        ...
        for node in op_nodes:
            ...
            tag_data = node.find(
                ".//{http://graphml.graphdrawing.org/xmlns}data[@key='d1']"
            )
            if tag_data is not None and tag_data.text:
                tagged_count += 1
                if len(sample_tags) < 5:
                    op_type_data = node.find(
                        ".//{http://graphml.graphdrawing.org/xmlns}data[@key='d0']"
                    )
                    op_type = (
                        op_type_data.text if op_type_data is not None else "Unknown"
                    )
