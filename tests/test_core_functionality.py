...
class TestModelInputGenerator:
    ...
    def test_manual_input_specs_basic(self):
        ...
        assert list(input_ids.shape) == [2, 32], (
            f"Expected [2, 32], got {list(input_ids.shape)}"
        )
        assert input_ids.dtype == torch.long, (
            f"Expected torch.long, got {input_ids.dtype}"
        )
        assert input_ids.min() >= 0 and input_ids.max() <= 1000, (
            "input_ids out of range [0, 1000]"
        )
        ...
        assert token_type_ids.min() >= 0 and token_type_ids.max() <= 1, (
            "token_type_ids out of range [0, 1]"
        )
        ...
        assert attention_mask.min() >= 0 and attention_mask.max() <= 1, (
            "attention_mask out of range [0, 1]"
        )
    ...
    def test_automatic_generation_bert_model(self):
        ...
        assert input_ids.dtype in (torch.long, torch.int64), (
            f"input_ids should be integer type, got {input_ids.dtype}"
        )
        assert len(input_ids.shape) >= 2, (
            f"input_ids should be at least 2D, got shape {input_ids.shape}"
        )
        ...
        for input_name in ["token_type_ids", "attention_mask"]:
            if input_name in inputs:
                ...
                assert isinstance(tensor, torch.Tensor), (
                    f"{input_name} should be torch.Tensor"
                )
                assert tensor.shape == input_ids.shape, (
                    f"{input_name} shape should match input_ids"
                )
    ...
    def test_sam_coordinate_generation(self):
        ...
        inputs = generate_dummy_inputs(
            model_name_or_path="facebook/sam-vit-base", task="mask-generation"
        )
        ...
        assert isinstance(input_points, torch.Tensor), (
            "input_points should be torch.Tensor"
        )
        assert input_points.dtype == torch.float32, (
            f"input_points should be float32, got {input_points.dtype}"
        )
        ...
        assert max_val < 2, (
            f"Coordinates should be normalized [0, 1], got max={max_val}"
        )

        # Validate shape (SAM expects [batch_size, point_batch_size, nb_points_per_image, 2])
        assert len(input_points.shape) == 4, (
            f"input_points should be 4D tensor, got shape {input_points.shape}"
        )
        assert input_points.shape[-1] == 2, (
            f"Last dimension should be 2 (x, y coordinates), got {input_points.shape[-1]}"
        )
    ...
    def test_input_specs_priority_over_auto_generation(self):
        ...
        assert set(inputs.keys()) == {"custom_input_1", "custom_input_2"}, (
            "Should only use input_specs, not model inputs"
        )
        ...
        assert 0.0 <= custom_1.min() <= custom_1.max() <= 1.0, (
            "Should respect range [0.0, 1.0]"
        )
        ...
        assert 0 <= custom_2.min() <= custom_2.max() <= 100, (
            "Should respect range [0, 100]"
        )
    ...
    def test_error_handling_invalid_specs(self):
        ...
        # Missing shape field
        with pytest.raises(
            ValueError, match="Missing 'shape' in input spec for 'bad_input'"
        ):
            generate_dummy_inputs(
                input_specs={"bad_input": {"dtype": "int", "range": [0, 100]}}
            )

        # Missing dtype field
        with pytest.raises(
            ValueError, match="Missing 'dtype' in input spec for 'bad_input'"
        ):
            generate_dummy_inputs(
                input_specs={"bad_input": {"shape": [1, 10], "range": [0, 100]}}
            )

        # Invalid dtype
        with pytest.raises(ValueError, match="Unsupported dtype"):
            generate_dummy_inputs(
                input_specs={"bad_input": {"shape": [1, 10], "dtype": "invalid_type"}}
            )

        # Invalid shape (not a list)
        with pytest.raises(ValueError, match="Shape must be a list"):
            generate_dummy_inputs(
                input_specs={"bad_input": {"shape": "not_a_list", "dtype": "int"}}
            )
...
class TestTracingHierarchyBuilder:
    ...
    def test_hierarchy_building_bert_model(self):
        ...
        assert isinstance(summary["execution_steps"], int), (
            "Execution steps should be integer"
        )
        ...
        root_modules = [
            name for name in module_names if "/" not in name or name.count("/") == 1
        ]
        ...
        assert len(nested_modules) > 0, (
            f"Should discover nested module hierarchy. Found {len(module_names)} total modules"
        )
        ...
        for module_path, module_info in hierarchy_data.items():
            assert isinstance(module_info, dict), (
                f"Module info should be dict for {module_path}"
            )

            # Should have basic information
            assert "module_type" in module_info, (
                f"Missing module_type for {module_path}"
            )
            assert isinstance(module_info["module_type"], str), (
                f"module_type should be string for {module_path}"
            )

            # Should track execution information
            if "execution_count" in module_info:
                assert isinstance(module_info["execution_count"], int), (
                    f"execution_count should be int for {module_path}"
                )
                assert module_info["execution_count"] >= 0, (
                    f"execution_count should be non-negative for {module_path}"
                )
    ...
    def test_hierarchy_building_custom_model(self):
        ...
        assert len(found_patterns) > 0, (
            f"Should find custom modules. Found paths: {list(module_paths)}"
        )
        ...
        for module_path, module_info in hierarchy_data.items():
            ...
            if "linear" in module_path.lower():
                assert class_name == "Linear", (
                    f"Expected Linear class name, got {class_name}"
                )
            elif (
                "custom_sub" in module_path and "." not in module_path
            ):  # Only the main custom_sub, not custom_sub.linear
                assert class_name == "CustomSubModule", (
                    f"Expected CustomSubModule class name, got {class_name}"
                )

            # All custom modules should be marked as pytorch type
            assert module_type == "pytorch", (
                f"Expected pytorch module type, got {module_type}"
            )
    ...
    def test_execution_context_capture(self):
        ...
        assert len(hierarchy_data) == 1, (
            f"Should only have root module (torch.nn excluded). Found: {list(hierarchy_data.keys())}"
        )

        # The root module should be TrackedModel
        root_info = hierarchy_data.get("", {})
        assert root_info.get("class_name") == "TrackedModel", (
            "Root should be TrackedModel"
        )
        ...
        assert len(linear_modules) == 2, (
            f"Should find both linear layers when exceptions used. Found: {[path for path, _ in linear_modules]}"
        )
...
class TestONNXNodeTagger:
    ...
    def test_node_tagger_creation_from_hierarchy(self):
        ...
        node_tagger = create_node_tagger_from_hierarchy(
            hierarchy_data, enable_operation_fallback=True
        )
        ...
        assert hasattr(node_tagger, "get_tagging_statistics"), (
            "Should have statistics method"
        )

        # Validate model root tag configuration
        assert hasattr(node_tagger, "model_root_tag"), "Should have model root tag"
        assert isinstance(node_tagger.model_root_tag, str), (
            "Model root tag should be string"
        )
        assert len(node_tagger.model_root_tag) > 0, "Model root tag should not be empty"

        # Root tag should reflect the model type
        assert (
            "BertModel" in node_tagger.model_root_tag
            or "AutoModel" in node_tagger.model_root_tag
    ...
    def test_node_tagging_coverage_validation(self):
        ...
        assert isinstance(tagged_nodes, dict), (
            "Should return dictionary of tagged nodes"
        )
        assert len(tagged_nodes) > 0, "Should tag at least one node"

        # Check for empty tags (CARDINAL RULE VIOLATION)
        empty_tags = [
            name for name, tag in tagged_nodes.items() if not tag or not tag.strip()
        ]
        assert len(empty_tags) == 0, (
            f"CARDINAL RULE VIOLATION: Found {len(empty_tags)} empty tags: {empty_tags}"
        )
        ...
        for node_name, tag in tagged_nodes.items():
            ...
            assert tag.startswith("/"), (
                f"Tag should start with '/' for hierarchical format, got: {tag}"
            )
        ...
        coverage_percentage = (
            (tagged_count / total_nodes * 100) if total_nodes > 0 else 0
        )

        assert coverage_percentage == 100.0, (
            f"Coverage should be 100%, got {coverage_percentage}%"
        )
    ...
    def test_tag_propagation_fallback_strategies(self):
        ...
        for node_name, tag in tagged_nodes.items():
            assert tag and tag.strip(), (
                f"Node {node_name} should have non-empty tag via fallback"
            )
            assert tag.startswith("/"), f"Tag should be hierarchical format: {tag}"

        # Validate statistics tracking
        total_strategy_uses = (
            stats.get("direct_matches", 0)
            + stats.get("parent_matches", 0)
            + stats.get("root_fallbacks", 0)
        )
        ...
        assert stats.get("root_fallbacks", 0) > 0, (
            "Should use root fallback for some nodes"
        )
...
class TestUtilityFunctions:
    ...
    def test_export_config_patching_system(self):
        ...
        assert patched_classes == original_classes, (
            "SAM config should not be patched by patch_export_config"
        )
        ...
        assert patched_bert_classes == original_bert_classes, (
            "BERT config should not be patched"
        )
    ...
    def test_configuration_integration(self):
        ...
        for model_path, model_type in configs_to_test:
            ...
            assert hasattr(config, "generate_dummy_inputs"), (
                f"{model_type} config should have input generation"
            )
            assert hasattr(config, "DUMMY_INPUT_GENERATOR_CLASSES"), (
                f"{model_type} config should have generator classes"
            )
    ...
    def test_cardinal_rules_validation(self):
        ...
        assert len(hierarchy_data) > 0, (
            "Should work with custom architecture (MUST-001: No hardcoding)"
        )
        ...
        custom_classes = [
            ...
            if "Custom" in name or "Conv2d" in name or "BatchNorm2d" in name
        ]
        assert len(custom_classes) > 0, (
            f"Should capture custom module classes (MUST-003: Universal design). Found classes: {class_names}"
        )

        # Test tag creation works universally
        node_tagger = create_node_tagger_from_hierarchy(hierarchy_data)
        assert node_tagger is not None, (
            "Should create tagger for any architecture (MUST-003: Universal design)"
        )
    ...
    def test_onnx_node_tagger_cardinal_rules_comprehensive(self):
        ...
        for node_name, tag in bert_tagged_nodes.items():
            ...
            assert tag.strip(), (
                f"CARDINAL RULE VIOLATION: Whitespace-only tag for node {node_name}"
            )
            assert tag.startswith("/"), (
                f"CARDINAL RULE VIOLATION: Invalid tag format for node {node_name}: {tag}"
            )
            assert len(tag) > 1, (
                f"CARDINAL RULE VIOLATION: Tag too short for node {node_name}: {tag}"
            )

        print(
            f"NO EMPTY TAGS validation passed: {len(bert_tagged_nodes)} nodes tagged with 100% coverage"
        )
        ...
        scope_test_cases = [
            ...
            (
                "/very/deep/nested/path/operation",
        ...
        root_tag = bert_tagger._find_tag_for_scope(
            "completely.unknown.nonexistent.scope"
        )
        assert root_tag == "/BertModel", (
            f"Priority 4 root fallback failed: expected '/BertModel', got '{root_tag}'"
        )
        assert root_tag != "", (
            "CARDINAL RULE VIOLATION: Root fallback returned empty tag"
        )
        assert root_tag.startswith("/"), (
            "CARDINAL RULE VIOLATION: Root fallback returned invalid format"
        )
        ...
        for stat_name in required_stats:
            ...
            assert isinstance(stats[stat_name], int), (
                f"Statistic {stat_name} should be integer"
            )
            assert stats[stat_name] >= 0, (
                f"Statistic {stat_name} should be non-negative"
            )
        ...
        assert stats["root_nodes"] + stats["scoped_nodes"] == stats["total_nodes"], (
            "Node categorization mismatch"
        )
        assert (
            stats["direct_matches"] + stats["parent_matches"] + stats["root_fallbacks"]
            >= stats["scoped_nodes"]
        ), "Tagging strategy accounting error"

        print("ONNX Node Tagger comprehensive CARDINAL RULES validation passed")
        print(
            f"Statistics: {stats['total_nodes']} total nodes, {stats['direct_matches']} direct matches, {stats['parent_matches']} parent matches, {stats['root_fallbacks']} root fallbacks"
        )
    ...
    def test_input_generator_detailed_validation(self):
        ...
        invalid_shape_specs = {"input_ids": {"shape": "invalid_shape", "dtype": "int"}}
        ...
        negative_shape_specs = {"input_ids": {"shape": [1, -128], "dtype": "int"}}
        ...
        assert multi_inputs["input_ids"].dtype == torch.int64, (
            "input_ids should be int64"
        )
        assert multi_inputs["pixel_values"].dtype == torch.float32, (
        ...
        priority_specs = {"custom_input": {"shape": [1, 64], "dtype": "float"}}

        # Should use input_specs even if model_name_or_path is provided
        priority_inputs = generate_dummy_inputs(
            model_name_or_path="prajjwal1/bert-tiny", input_specs=priority_specs
        )
        ...
        assert "input_ids" not in priority_inputs, (
            "Should not auto-generate when input_specs provided"
        )

        print(
            "Detailed input generator validation passed: comprehensive error handling and edge cases covered"
        )
