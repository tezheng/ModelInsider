...
class TestSAMExportRegression:
    ...
    def test_dict_inputs_preserved_for_export(self):
        ...
        with patch("torch.onnx.export", side_effect=capture_export):
            with patch(
                "modelexport.core.onnx_utils.infer_output_names", return_value=None
            ):
        ...
        assert isinstance(captured_inputs, dict), (
            f"Expected dict inputs, got {type(captured_inputs)}"
        )
        assert captured_inputs is mock_dict_inputs, (
            "Dict inputs should be passed directly, not converted"
        )
    ...
    def test_tuple_inputs_preserved(self):
        ...
        assert isinstance(captured_inputs, tuple), (
            f"Expected tuple inputs, got {type(captured_inputs)}"
        )
        assert captured_inputs is mock_tuple_inputs, (
            "Tuple inputs should be passed directly"
        )
    ...
    def test_list_inputs_preserved(self):
        ...
        assert isinstance(captured_inputs, list), (
            f"Expected list inputs, got {type(captured_inputs)}"
        )
        assert captured_inputs is mock_list_inputs, (
            "List inputs should be passed directly"
        )
