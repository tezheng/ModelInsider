...
class TestONNXNodeTagger:
    ...
    def test_no_empty_tags_guaranteed(self, sample_hierarchy_data, mock_onnx_model):
        ...
        for node_name, tag in tagged_nodes.items():
            ...
            assert tag.startswith('/'), f"Invalid tag format for node {node_name}: {tag}"
    
    def test_root_nodes_get_clean_root_tag(self, sample_hierarchy_data, mock_onnx_model):
    ...
    def test_factory_function(self, sample_hierarchy_data):
        ...
        tagger2 = create_node_tagger_from_hierarchy(sample_hierarchy_data, enable_operation_fallback=True)
    ...
    def test_tagging_statistics(self, sample_hierarchy_data, mock_onnx_model):
        ...
        required_keys = ['total_nodes', 'root_nodes', 'scoped_nodes', 'unique_scopes',
                        'direct_matches', 'parent_matches', 'operation_matches', 'root_fallbacks']
...
class TestIntegrationWithTracingHierarchyBuilder:
    ...
    def test_integration_with_real_hierarchy_data(self):
        ...
        assert tag == "/BertModel/BertEncoder/BertLayer.0/BertAttention/BertSelfAttention/Query"