...
class TestGraphMLStructuralValidation:
    ...
    def test_1_xml_schema_compliance(self, sample_graphml_export):
        ...
        assert expected_namespace in root.tag or expected_namespace in root.attrib.values(), \
        ...
        assert len(missing_keys) == 0, f"Missing required key definitions: {missing_keys}"
        
        print(f"✅ XML Schema Compliance: Valid GraphML with {len(key_ids)} key definitions")
    ...
    def test_2_hanging_node_detection(self, sample_graphml_export):
        ...
        direct_nodes = main_graph.findall('./node') + main_graph.findall('./graphml:node', ns)
        suspicious_nodes = []
        ...
        for node in all_nodes:
            ...
            if node_id:
                ...
                if parent is not None:
                    parent_id = parent.get('id') if parent.tag.endswith('graph') else f"<{parent.tag}>"
        ...
        for node_id, locations in node_locations.items():
            if len(locations) > 1:
                print(f"🔍 DUPLICATE: Node {node_id} appears in {len(locations)} locations: {locations}")
                ...
                if len(path_parts) > 1:
                    ...
                    if expected_parent not in actual_parent and actual_parent not in expected_parent:
                        print(f"🔍 MISPLACED: Node {node_id} in '{actual_parent}' but should be in '{expected_parent}'")
                        suspicious_nodes.append({
                            'node_id': node_id,
                            'issue': 'misplaced',
        ...
        if suspicious_nodes:
            ...
            for node in suspicious_nodes:
                if node['issue'] == 'duplicate':
                    print(f"   - DUPLICATE: {node['node_id']} in {len(node['locations'])} locations: {node['locations']}")
                elif node['issue'] == 'misplaced':
                    print(f"   - MISPLACED: {node['node_id']} in '{node['actual_parent']}' should be in '{node['expected_parent']}'")
        
        # This test is expected to FAIL initially to expose the bug
        assert len(suspicious_nodes) == 0, f"Found structural issues in GraphML: {suspicious_nodes}"
    ...
    def test_3_graph_nesting_structure(self, sample_graphml_export):
            ...
            if parent.tag != 'graph':
                ...
                if '/' in node_id and node_id.startswith('/'):
                    ...
                    if len(path_parts) > 1:
                        ...
                        if (expected_context not in parent_graph_id and 
                            parent_graph_id not in expected_context and
                            parent_graph_id != expected_context):
                            nesting_violations.append({
                                'node_id': node_id,
        ...
        assert len(nesting_violations) == 0, f"Graph nesting violations: {nesting_violations}"
    ...
    def test_4_hierarchy_tag_validation(self, sample_graphml_export):
        ...
        print(f"📊 Found {len(operation_nodes)} operation nodes, {len(nodes_without_hierarchy)} missing hierarchy tags")
        
        assert len(nodes_without_hierarchy) == 0, f"Operation nodes missing hierarchy tags: {nodes_without_hierarchy}"
    ...
    def test_6_input_output_inclusion(self, sample_graphml_export):
        ...
        if graphml_has_input_metadata:
            try:
                ...
                assert len(missing_inputs) == 0, f"Missing inputs in GraphML: {missing_inputs}"
        ...
        if graphml_has_output_metadata:
            try:
                ...
                missing_outputs = [out for out in onnx_outputs if out not in graphml_output_names]
                assert len(missing_outputs) == 0, f"Missing outputs in GraphML: {missing_outputs}"
            except json.JSONDecodeError:
                pytest.fail("Output metadata is not valid JSON")

    @pytest.mark.integration
    def test_7_round_trip_structural_preservation(self, sample_graphml_export, cli_runner, temp_workspace):
        ...
        assert round_trip_result.exit_code == 0, f"Round-trip failed: {round_trip_result.output}"
...
class TestGraphMLEnhancedE2E:
    ...
    def test_comprehensive_e2e_structural_validation(self, cli_runner, temp_workspace):
        ...
        self._validate_node_count_preservation(expected_files['onnx'], expected_files['graphml'])
        self._validate_input_output_inclusion(expected_files['onnx'], expected_files['graphml'])
        
        # Enhanced round-trip validation
        reconstructed_path = temp_workspace['exports'] / 'bert_reconstructed_enhanced_e2e.onnx'
        round_trip_result = cli_runner.invoke(cli, [
            'import-onnx',
        ...
        self._validate_round_trip_preservation(expected_files['onnx'], reconstructed_path)
    ...
    def _validate_proper_nesting(self, graphml_path):
        ...
        for node in nodes:
            ...
            parent = parent_map.get(node)
            if parent is None or (parent.tag != 'graph' and not parent.tag.endswith('graph')):
    ...
    def _validate_hierarchy_tags(self, graphml_path):
        ...
        assert len(nodes_without_hierarchy) == 0, f"Missing hierarchy tags: {nodes_without_hierarchy}"
    ...
    def _validate_node_count_preservation(self, onnx_path, graphml_path):
        ...
        for node in nodes:
            # Skip compound nodes (nodes with nested graphs)
            nested_graph = node.find('./gml:graph', namespaces)
            if nested_graph is None:
                nested_graph = node.find('./graph')
            if nested_graph is not None:
                continue  # Skip compound nodes
                
            # Try namespace-aware search first, then fallback
            op_type_elem = node.find('./gml:data[@key="n0"]', namespaces)
            if op_type_elem is None:
                op_type_elem = node.find('./data[@key="n0"]')
            if op_type_elem is not None and op_type_elem.text:
                node_id = node.get('id')
                # Skip I/O metadata nodes that are added by GraphML v1.1 but not in ONNX
                if node_id and not (node_id.startswith('input_') or node_id.startswith('output_')):
                    graphml_operation_nodes.append(node_id)
    ...
    def _validate_round_trip_preservation(self, original_path, reconstructed_path):
        ...
        assert node_diff <= node_tolerance, f"Round-trip node count mismatch: {original_node_count} → {reconstructed_node_count}"
        
        assert len(original_model.graph.input) == len(reconstructed_model.graph.input), "Input count mismatch"
        assert len(original_model.graph.output) == len(reconstructed_model.graph.output), "Output count mismatch"
