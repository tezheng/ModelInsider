...
class TestCLIExport:
    ...
    def test_export_bert_tiny(self, cli_runner, temp_workspace):
        ...
        assert "node_tagging" in metadata["report"]["steps"], (
            "Missing node_tagging in report steps"
        )
    ...
    def test_export_nonexistent_model(self, cli_runner, temp_workspace):
        ...
        result = cli_runner.invoke(
            cli,
            ["export", "--model", "nonexistent/model", "--output", str(output_path)],
        )
    ...
    def test_export_with_graphml_large_model_handling(self, cli_runner, temp_workspace):
        ...
        result = cli_runner.invoke(
            ...
            [
                "export",
                "--model",
                "prajjwal1/bert-tiny",  # Using bert-tiny but could use larger
                "--output",
                str(output_path),
                "--with-graphml",
        ...
        graphml_path = (
            output_path.parent / f"{output_path.stem}_hierarchical_graph.graphml"
        )
        params_path = (
            output_path.parent / f"{output_path.stem}_hierarchical_graph.onnxdata"
        )
        ...
        assert 0.8 < size_ratio < 1.2, (
            f"Parameter file size unexpected: {size_ratio:.2f}x ONNX size"
        )

    def test_export_with_graphml_permission_error_recovery(
        self, cli_runner, temp_workspace
    ):
    ...
    def test_export_with_graphml_concurrent_exports(self, cli_runner, temp_workspace):
        ...
        for output_path in [output_path1, output_path2]:
            base_name = output_path.stem
            graphml_path = (
                output_path.parent / f"{base_name}_hierarchical_graph.graphml"
            )
            params_path = (
                output_path.parent / f"{base_name}_hierarchical_graph.onnxdata"
            )

            assert graphml_path.exists()
            assert params_path.exists()

    def test_export_with_graphml_disk_space_simulation(
        self, cli_runner, temp_workspace
    ):
    ...
    def test_export_with_graphml_model_architecture_variety(
        self, cli_runner, temp_workspace
    ):
    ...
    def test_export_with_graphml_invalid_output_path(self, cli_runner, temp_workspace):
        ...
        output_path = (
            temp_workspace["exports"] / "nonexistent" / "subdir" / "model.onnx"
        )
    ...
    def test_export_with_graphml_validate_sidecar_format(
        self, cli_runner, temp_workspace
    ):
    ...
    def test_export_with_graphml_partial_failure_recovery(
        self, cli_runner, temp_workspace
    ):
...
class TestCLIAnalyze:
    ...
    def test_analyze_summary(self, cli_runner, sample_onnx_model):
        ...
        result = cli_runner.invoke(
            cli, ["analyze", str(sample_onnx_model), "--output-format", "summary"]
        )
    ...
    def test_analyze_csv_output(self, cli_runner, sample_onnx_model, temp_workspace):
        ...
        result = cli_runner.invoke(
            cli, ["analyze", str(sample_onnx_model), "--output-format", "csv"]
        )
    ...
    def test_analyze_nonexistent_file(self, cli_runner):
        ...
        result = cli_runner.invoke(cli, ["analyze", "nonexistent.onnx"])
...
class TestCLIValidate:
    ...
    def test_validate_basic(self, cli_runner, sample_onnx_model):
        ...
        result = cli_runner.invoke(cli, ["validate", str(sample_onnx_model)])

        assert result.exit_code == 0
        assert "✅ ONNX model is valid" in result.output  # Works for both messages
        assert (
            "Found" in result.output
            and "operations with hierarchy tags" in result.output
        )
    ...
    def test_validate_consistency_check(self, cli_runner, sample_onnx_model):
        ...
        result = cli_runner.invoke(
            cli,
            ["--verbose", "validate", str(sample_onnx_model), "--check-consistency"],
        )
        ...
        assert any(
            ...
            for phrase in ["✅ Tags are consistent", "❌ Tag inconsistencies found"]
        )

    def test_validate_nonexistent_file(self, cli_runner):
        """Test validation of nonexistent file."""
        result = cli_runner.invoke(cli, ["validate", "nonexistent.onnx"])
...
class TestCLICompare:
    ...
    def two_sample_models(self, temp_workspace):
        ...
        inputs2 = tokenizer(
            "Second model with different input text", return_tensors="pt"
        )
    ...
    def test_compare_models(self, cli_runner, two_sample_models):
        ...
        result = cli_runner.invoke(cli, ["compare", str(path1), str(path2)])
    ...
    def test_compare_to_file(self, cli_runner, two_sample_models, temp_workspace):
        ...
        result = cli_runner.invoke(
            cli, ["compare", str(path1), str(path2), "--output-file", str(output_file)]
        )
    ...
    def test_compare_nonexistent_files(self, cli_runner):
        ...
        result = cli_runner.invoke(
            cli, ["compare", "nonexistent1.onnx", "nonexistent2.onnx"]
        )
...
class TestCLIIntegration:
    ...
    def test_export_analyze_validate_workflow(self, cli_runner, temp_workspace):
        ...
        export_result = cli_runner.invoke(
            cli,
            ["export", "--model", "prajjwal1/bert-tiny", "--output", str(model_path)],
        )
        ...
        analyze_result = cli_runner.invoke(
            cli, ["analyze", str(model_path), "--output-format", "summary"]
        )
        ...
        validate_result = cli_runner.invoke(
            cli, ["validate", str(model_path), "--check-consistency"]
        )
