    ...
    def test_no_hardcoded_windows_paths(self):
        ...
        for test_file in test_files:
            if Path(test_file).exists():
                ...
                assert "/mnt/c/" not in content, (
                    f"WSL Windows mount found in {test_file}"
                )
            ...
            for i, line in enumerate(lines):
                if (
                    ...
                    raise AssertionError(
                        f"Hardcoded 'python' found in {test_file} line {i + 1}: {line}"
                    )
    ...
    def test_subprocess_calls_use_current_interpreter(self):
        ...
        test_methods = [
            ...
            for name, method in inspect.getmembers(
                test_cli_integration.TestCLIProcessIntegration
            )
