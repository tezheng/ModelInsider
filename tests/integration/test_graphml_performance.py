...
class TestGraphMLPerformance:
    ...
    def test_graphml_overhead_bert_tiny(self, cli_runner, temp_workspace):
        ...
        cli_runner.invoke(
            cli,
            ["export", "--model", "prajjwal1/bert-tiny", "--output", str(warmup_path)],
        )

        # Benchmark without GraphML
        output_path_no_graphml = temp_workspace / "bert_no_graphml.onnx"
        start_time = time.time()
        result_no_graphml = cli_runner.invoke(
            cli,
            [
                "export",
                "--model",
        ...
        start_time = time.time()
        result_with_graphml = cli_runner.invoke(
            cli,
            [
                "export",
                "--model",
        ...
        graphml_path = (
            output_path_with_graphml.parent
            / f"{output_path_with_graphml.stem}_hierarchical_graph.graphml"
        )
        params_path = (
            output_path_with_graphml.parent
            / f"{output_path_with_graphml.stem}_hierarchical_graph.onnxdata"
        )
        ...
        print(
            f"Parameters size: {params_size / 1024 / 1024:.1f} MB ({params_ratio:.1f}% of ONNX)"
        )

        # Size assertions
        assert graphml_ratio < 30, (
            f"GraphML file too large: {graphml_ratio:.1f}% of ONNX"
        )
        assert 80 < params_ratio < 120, (
            f"Parameter file size unexpected: {params_ratio:.1f}% of ONNX"
        )
    ...
    def test_graphml_overhead_multiple_models(
        self, cli_runner, temp_workspace, model_name, expected_overhead
    ):
        ...
        result = cli_runner.invoke(
            cli, ["export", "--model", model_name, "--output", str(output_path)]
        )
        ...
        output_path_graphml = (
            temp_workspace / f"{model_name.replace('/', '_')}_graphml.onnx"
        )
        start_time = time.time()
        result = cli_runner.invoke(
            cli,
