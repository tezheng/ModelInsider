...
class TestSAMInputGenerationFix:
    ...
    def test_sam_generates_pixel_values(self):
        ...
        inputs = generate_dummy_inputs_from_model_path('facebook/sam-vit-base', task='feature-extraction')
        ...
        assert 'image_embeddings' not in inputs, "SAM encoder should not have embeddings input"
        assert 'image_positional_embeddings' not in inputs, "SAM encoder should not have positional embeddings input"
    ...
    def test_sam_inputs_work_with_model(self):
        ...
        encoder_inputs = generate_dummy_inputs_from_model_path('facebook/sam-vit-base', task='feature-extraction')
        ...
        with torch.no_grad():
            ...
            try:
                ...
                assert hasattr(image_embeddings, 'last_hidden_state') or isinstance(image_embeddings, torch.Tensor), \
                    "Vision encoder should produce embeddings"
                    
            except Exception as e:
                pytest.fail(f"Generated encoder inputs failed vision encoder inference: {e}")
            
        # Test 2: Decoder inputs structure
        decoder_inputs = generate_dummy_inputs_from_model_path('facebook/sam-vit-base', task='mask-generation')
        
        # Validate decoder inputs have correct structure
        assert 'image_embeddings' in decoder_inputs, "Decoder should have image_embeddings"
        assert 'image_positional_embeddings' in decoder_inputs, "Decoder should have positional embeddings"
        ...
        assert img_emb.shape == pos_emb.shape, "Image and positional embeddings should have same shape"
    ...
    def test_sam_export_task_support(self):
        ...
        assert hasattr(encoder_config, 'vision_encoder'), "Should have vision_encoder attribute"
        assert encoder_config.vision_encoder == True, "feature-extraction should set vision_encoder=True"
        ...
        assert hasattr(decoder_config, 'vision_encoder'), "Should have vision_encoder attribute"
        assert decoder_config.vision_encoder == False, "mask-generation should set vision_encoder=False"
...
class TestSAMInputGenerationRegression:
    ...
    def test_sam_does_not_generate_embeddings(self):
        ...
        for decoder_input in decoder_inputs:
            assert decoder_input not in inputs, f"Encoder should not have decoder input: {decoder_input}"
