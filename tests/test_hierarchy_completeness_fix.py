...
class TestHierarchyCompletenessFix:
    ...
    def test_tracing_hierarchy_builder_excludes_torch_nn_by_default(self):
        ...
        found_torch_nn_classes = [
            cls for cls in class_names if cls in forbidden_torch_nn_classes
        ]
        ...
        assert len(hierarchy) >= 15, (
            f"Expected at least 15 HF modules, got {len(hierarchy)}"
        )
        ...
        for module_name, info in hierarchy.items():
            ...
            assert info["traced_tag"].startswith("/"), (
                f"Invalid tag format: {info['traced_tag']}"
            )
        ...
        print(
            f"✅ MUST-002 compliant: {len(hierarchy)} HF modules, no torch.nn modules"
        )
    ...
    def test_htp_export_generates_complete_reports(self, tmp_path):
        ...
        assert total_modules >= 15, (
            f"Expected at least 15 HF modules in metadata, got {total_modules}"
        )
        ...
        forbidden_torch_nn_classes = ["LayerNorm", "Linear", "Dropout", "Embedding"]
        found_torch_nn_classes = [
            cls for cls in all_class_names if cls in forbidden_torch_nn_classes
        ]
    ...
    def test_hierarchy_contains_expected_bert_structure(self):
        ...
        assert len(nested_paths) > 0, (
            f"Expected nested hierarchy paths, got: {tagged_paths}"
        )
