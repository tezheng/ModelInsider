...
def bert_tiny_artifacts(tmp_path):
    ...
    token_type_ids = helper.make_tensor_value_info("token_type_ids", TensorProto.INT64, [2, 16])
    
    # Model outputs  
    last_hidden_state = helper.make_tensor_value_info("last_hidden_state", TensorProto.FLOAT, [2, 16, 128])
    pooler_output = helper.make_tensor_value_info("pooler_output", TensorProto.FLOAT, [2, 128])
