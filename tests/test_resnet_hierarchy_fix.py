...
class TestResNetHierarchyFix:
    ...
    def test_compound_pattern_detection(self):
        ...
        assert root_children == expected_root, (
            f"Expected {expected_root}, got {root_children}"
        )
        ...
        expected_layer0 = [
            "encoder.layer.0.bn1",
            "encoder.layer.0.conv1",
        ]  # Alphabetical order
        assert layer0_children == expected_layer0, (
            f"Expected {expected_layer0}, got {layer0_children}"
        )
    ...
    def test_resnet_ascii_tree_generation(self):
        ...
        assert "ResNetLayer: encoder.layer.0" in tree_text, (
            "Missing layer.0 under encoder"
        )
        assert "ResNetLayer: encoder.layer.1" in tree_text, (
            "Missing layer.1 under encoder"
        )
    ...
    def test_mixed_patterns(self):
        ...
        assert encoder_children == expected, (
            f"Expected {expected}, got {encoder_children}"
        )
    ...
    @pytest.mark.skipif(
        True,
        reason="Requires transformers with ResNet models - enable for full testing",
    )
    def test_real_resnet_model_export(self, tmp_path):
        ...
        print(
            f"✅ Real ResNet export successful with {len(encoder_children)} encoder layers"
        )
