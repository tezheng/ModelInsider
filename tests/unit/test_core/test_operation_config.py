...
class TestOperationConfig:
    ...
    def test_operation_entry_structure(self):
        ...
        for op_name, op_config in registry.items():
            ...
            assert "patch_targets" in op_config, (
                f"Operation {op_name} missing patch_targets"
            )
            ...
            for target in op_config["patch_targets"]:
                if target:  # Some operations have empty patch targets
                    assert isinstance(target, tuple), (
                        f"Patch target should be tuple for {op_name}"
                    )
                    assert len(target) == 2, (
                        f"Patch target should have 2 elements for {op_name}"
                    )

                    module_name, function_name = target
                    assert isinstance(module_name, str)
                    assert isinstance(function_name, str)
                    assert module_name in ["torch", "F"], (
                        f"Unknown module {module_name} for {op_name}"
                    )
    ...
    def test_onnx_types_validity(self):
        ...
        for op_name, op_config in registry.items():
            for onnx_type in op_config["onnx_types"]:
                assert isinstance(onnx_type, str), (
                    f"ONNX type should be string for {op_name}"
                )
    ...
    def test_get_torch_to_onnx_mapping(self):
        ...
        for op_name, expected_onnx_types in expected_mappings.items():
            ...
            for onnx_type in expected_onnx_types:
                assert onnx_type in actual_onnx_types, (
                    f"{onnx_type} should be in mapping for {op_name}"
                )
    ...
    def test_priority_ordering(self):
        ...
        for op in core_ops:
            if op in registry:
                assert registry[op]["priority"] <= 3, (
                    f"Core op {op} should have high priority"
                )
        ...
        for op in advanced_ops:
            if op in registry:
                assert registry[op]["priority"] >= 3, (
                    f"Advanced op {op} should have lower priority"
                )
    ...
    def test_operation_categories(self):
        ...
        for op in math_ops:
            if op in registry:
                assert registry[op]["priority"] <= 3, (
                    f"Math op {op} should have high priority"
                )
        ...
        for op in shape_ops:
            if op in registry:
                assert registry[op]["priority"] >= 2, (
                    f"Shape op {op} should have medium priority"
                )
        ...
        for op in reduction_ops:
            if op in registry:
                assert registry[op]["priority"] >= 3, (
                    f"Reduction op {op} should have medium-low priority"
                )
    ...
    def test_operation_coverage(self):
        ...
        essential_ops = ["matmul", "add", "relu", "embedding", "reshape", "transpose"]
...
class TestOperationConfigIntegration:
    ...
    def test_config_used_by_strategies(self):
        ...
        assert len(common_ops) > 0, (
            "Should have operations in both patch list and mapping"
        )
