...
class HTPIntegratedExporterWithReporting(HTPIntegratedExporter):
    ...
    def _report_hierarchy_building(self, execution_summary: Dict[str, Any]) -> None:
        ...
        if total_modules > 0:
            ...
            self._print_and_log(
                f"   Optimization ratio: {hierarchy_count}/{total_modules} ({optimization_ratio:.1f}%)"
            )
    ...
    def _print_hierarchy_tree(self, hierarchy_data: Dict[str, Dict]) -> None:
        ...
        sorted_modules = sorted(
            hierarchy_data.items(), key=lambda x: x[1].get("execution_order", 999)
        )
        ...
        if self.verbose:
            ...
            tree = Tree(
                Text(root_class, style="bold bright_cyan"), guide_style="bright_white"
            )
            ...
            # Simple text output for report matching verbose format
            tree_lines = []
            tree_lines.append(f"{root_class}")

            # Build proper tree structure similar to verbose version
            for module_path, module_info in sorted_modules:
                if not module_path:
                    continue
                ...
                # Use proper tree formatting similar to verbose
                if depth == 0:
                    prefix = "├── "
                else:
                    prefix = "│   " * (depth - 1) + "├── "
                
                tree_lines.append(f"{prefix}{class_name}: {module_path}")

            # Fix last items to use └── instead of ├──
            if len(tree_lines) > 1:
    ...
    def _report_onnx_export(
        self, onnx_model: onnx.ModelProto, output_path: str
    ) -> None:
    ...
    def _report_node_tagger_creation(self) -> None:
        ...
        # Store node tagger configuration for report
        self._detailed_stats["node_tagger_config"] = {
            "model_root": self._node_tagger.model_root_tag,
            "scope_mappings": len(self._node_tagger.scope_to_tag),
            "operation_fallback": "Enabled" if self._node_tagger.enable_operation_fallback else "Disabled"
        }
        ...
        fallback_status = (
            "Enabled" if self._node_tagger.enable_operation_fallback else "Disabled"
        )
    ...
    def _report_node_tagging(self, onnx_model: onnx.ModelProto) -> None:
        ...
        accuracy = {
            "direct_match_rate": (stats["direct_matches"] / total * 100)
            if total > 0
            else 0,
            "parent_match_rate": (stats["parent_matches"] / total * 100)
            if total > 0
            else 0,
            "root_fallback_rate": (stats["root_fallbacks"] / total * 100)
            if total > 0
            else 0,
            "operation_match_rate": (stats["operation_matches"] / total * 100)
            if total > 0
            else 0,
        }
        ...
        self._print_and_log("\n📊 Tagging Statistics:")
        self._print_and_log(
            f"   Direct matches: {stats['direct_matches']} ({accuracy['direct_match_rate']:.1f}%)"
        )
        self._print_and_log(
            f"   Parent matches: {stats['parent_matches']} ({accuracy['parent_match_rate']:.1f}%)"
        )
        self._print_and_log(
            f"   Root fallbacks: {stats['root_fallbacks']} ({accuracy['root_fallback_rate']:.1f}%)"
        )

        if self._node_tagger.enable_operation_fallback:
            self._print_and_log(
                f"   Operation matches: {stats['operation_matches']} ({accuracy['operation_match_rate']:.1f}%)"
            )
    ...
    def _analyze_scope_buckets(self, onnx_model: onnx.ModelProto) -> None:
        ...
        sorted_scopes = sorted(
            simple_buckets.items(), key=lambda x: len(x[1]), reverse=True
        )
    ...
    def _print_complete_hierarchy_with_nodes(self, onnx_model: onnx.ModelProto) -> None:
        ...
        if not self.verbose:
            self._print_and_log(
                "🌳 Complete hierarchy tree (see detailed version in report file)"
            )
    ...
    def _generate_complete_hierarchy_tree(
        self, onnx_model: onnx.ModelProto, show_console: bool = True
    ) -> None:
        ...
        tree = Tree(
            Text(
                f"{root_class} ({len(self._tagged_nodes)} ONNX nodes)",
        ...
        sorted_hierarchy = sorted(
            self._hierarchy_data.items(), key=lambda x: x[1].get("execution_order", 999)
        )
            ...
            if module_nodes:
                ...
                for op_type, op_nodes in sorted(ops_by_type.items()):
                    if len(op_nodes) == 1:
                        ...
                        group_text.append(
                            f" ({len(op_nodes)} ops)", style="dim bright_white"
                        )
        ...
        if show_console:
            self.console.print(tree)
            # Add the actual tree content to report buffer for console output section
            self._print_and_log(tree_text)
        else:
    ...
    def _add_simplified_tree_to_report(self) -> None:
        ...
        for module_path, module_info in sorted_hierarchy:
            ...
            indent = "  " * depth
            tree_lines.append(
                f"{indent}└─ {class_name}: {module_path} ({node_count} nodes)"
            )
            ...
            if module_nodes:
                ...
                top_ops = sorted(ops_by_type.items(), key=lambda x: x[1], reverse=True)[
                    :3
                ]
    ...
    def _generate_export_report(self, output_path: str) -> None:
        ...
        # Add node tagger configuration
        if self._detailed_stats.get("node_tagger_config"):
            report_content.append("📋 ONNX Node Tagging Analysis:")
            report_content.append("-" * 30)
            config = self._detailed_stats["node_tagger_config"]
            report_content.append(f"Model root: {config['model_root']}")
            report_content.append(f"Scope mappings: {config['scope_mappings']}")
            report_content.append(f"Operation fallback: {config['operation_fallback']}")
            report_content.append("")

        if self._detailed_stats.get("tagging_statistics"):
            report_content.append("🏷️ Tagging Statistics:")
            report_content.append("-" * 30)
        ...
        if self._detailed_stats.get("tag_distribution"):
            report_content.append("🏷️ Top 20 Tags by Node Count:")
            report_content.append("-" * 30)
            total_nodes = sum(self._detailed_stats["tag_distribution"].values())
            for i, (tag, count) in enumerate(
                list(self._detailed_stats["tag_distribution"].items())[:20]
            ):
                percentage = (count / total_nodes) * 100
                report_content.append(
                    f"{i + 1:2d}. {tag}: {count} nodes ({percentage:.1f}%)"
                )
            report_content.append("")

        # Add detailed analysis (moved to end)
        if self._detailed_stats.get("execution_summary"):
            report_content.append("⚡ Execution Summary:")
            report_content.append("-" * 30)
            exec_summary = self._detailed_stats["execution_summary"]
            for key, value in exec_summary.items():
                if key in ["hierarchy_mapping", "module_hierarchy"]:
                    import json
                    report_content.append(f"{key}: {json.dumps(value, indent=2)}")
                else:
                    report_content.append(f"{key}: {value}")
            report_content.append("")
...
def export_with_htp_integrated_reporting(
    ...
    **kwargs,
    ...
    Returns:
        Enhanced export statistics with reporting data
    """
    exporter = HTPIntegratedExporterWithReporting(
        verbose=verbose, enable_reporting=True
    )
