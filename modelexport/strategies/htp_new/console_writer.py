...
class ConsoleWriter(StepAwareWriter):
    ...
    def write_model_prep(self, export_step: ExportStep, data: ExportData) -> int:
        ...
        self.console.print(f"\n📋 {self._bold(f'STEP {self._current_step}/{self._total_steps}: MODEL PREPARATION')}")
    ...
    def write_input_gen(self, export_step: ExportStep, data: ExportData) -> int:
        ...
        if data.input_gen.method == "provided":
            ...
            if data.input_gen.model_type:
                self.console.print(f"   • Model type: {self._bright_green(data.input_gen.model_type)}")
    ...
    def write_hierarchy(self, export_step: ExportStep, data: ExportData) -> int:
        ...
        self.console.print(f"\n🏗️ {self._bold(f'STEP {self._current_step}/{self._total_steps}: HIERARCHY BUILDING')}")
        self.console.print("=" * self.SEPARATOR_LENGTH)
    ...
    def write_onnx_export(self, export_step: ExportStep, data: ExportData) -> int:
        ...
        self.console.print(f"   • Opset version: {self._bright_green(data.onnx_export.opset_version)}")
        self.console.print(f"   • Constant folding: {self._format_bool(data.onnx_export.do_constant_folding)}")
        ...
        if data.onnx_export.output_names:
            ...
            self.console.print(
                f"⚠️  Output names: {self._bright_yellow('Not detected')} "
                "(model may not have named outputs)"
            )
        
        self.console.print("✅ ONNX model exported successfully")
        self.console.print(f"📦 Model size: {self._bright_cyan(f'{data.onnx_export.onnx_size_mb:.2f}MB')}")
    ...
    def write_node_tagging(self, export_step: ExportStep, data: ExportData) -> int:
        ...
        self.console.print(f"\n🔗 {self._bold(f'STEP {self._current_step}/{self._total_steps}: ONNX NODE TAGGING')}")
        self.console.print("=" * self.SEPARATOR_LENGTH)
        
        self.console.print("✅ Node tagging completed successfully")
        self.console.print(f"📈 Coverage: {self._bright_cyan(f'{data.node_tagging.coverage:.1f}%')}")
        self.console.print(
