...
from ...core.operation_config import OperationConfig
...
class UsageBasedExporter(BaseHierarchyExporter):
    ...
    def __init__(self, torch_nn_exceptions: Optional[List[str]] = None):
        """
        ...
        """
        super().__init__()
        self._torch_nn_exceptions = torch_nn_exceptions or ["LayerNorm"]
    ...
    def _track_module_usage(
        ...
        for name, module in all_modules.items():
            if should_tag_module(module, self._torch_nn_exceptions):
                ...
                def create_usage_hook(module_name: str):
                ...
                hook = module.register_forward_hook(create_usage_hook(name))
                hooks.append(hook)
        
        try:
            # Run forward pass to trigger hooks
            with torch.no_grad():
                model(*example_inputs if isinstance(example_inputs, (list, tuple)) else [example_inputs])
        finally:
            # Remove hooks
            for hook in hooks:
                hook.remove()
