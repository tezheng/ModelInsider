...
class GraphMLToONNXConverter:
    ...
    def _create_onnx_node(self, node_data: Dict[str, Any]) -> helper.NodeProto:
        ...
        # Convert attributes (filter out custom attributes)
        attributes = []
        onnx_attrs = node_data.get("onnx_attributes", {})
        for attr_name, attr_value in onnx_attrs.items():
            # Skip custom attributes that are not valid ONNX attributes
            if not self._is_custom_attribute(attr_name):
    ...
    def _is_custom_attribute(self, attr_name: str) -> bool:
        ...
        # Custom attributes that we add but shouldn't be in ONNX
        custom_attrs = {
